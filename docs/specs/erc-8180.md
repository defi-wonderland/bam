...
[Blob/calldata] → registerBlobBatch / registerBlobBatches / registerCalldataBatch (Core)
                     ↓ emits one BlobBatchRegistered(decoder, registry) per registered batch
...
19. **`registerBlobBatches` is OPTIONAL during the transition.** The interface declares the
    function; implementations MAY ship without it during a deployment window in which only
    `registerBlobBatch` is needed. Producers that pack multiple per-tag batches in a single
    transaction MUST use `registerBlobBatches` (no other entrypoint provides cross-entry
    atomicity); single-entry callers MAY use either entrypoint with byte-identical event content.
