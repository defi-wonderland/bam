...
[Blob/calldata] → registerBlobBatch / registerBlobBatches / registerCalldataBatch (Core)
                     ↓ emits BlobBatchRegistered (blob path) or CalldataBatchRegistered
                     ↓ (calldata path), one per registered batch
...
14. **Indexed-topic layout.** In both BAM core events, `contentTag` occupies the third indexed
    ...
    event directly, at parity with ERC-BSS's `BlobSegmentDeclared`. This is a breaking change
...
19. **`registerBlobBatches` conformance.** Implementations claiming ERC-8180 conformance MUST
...
##### Reference v1 ABI shape

The reference implementation in this repository (`ABIDecoder.sol`) uses Solidity ABI encoding with
the following tuple, paired with scheme `0x01` (ECDSA-secp256k1):

```solidity
payload = abi.encode(
    Message[] messages,    // tuple[](address sender, uint64 nonce, bytes contents)
    bytes signatureData    // concat(sig_0, sig_1, …, sig_{n-1})
)
```

For the ECDSA pairing, `signatureData` is the byte-concatenation of `n` fixed-width 65-byte
signatures, so `len(signatureData) == 65 * messages.length` and `sig_i == signatureData[i*65 : (i+1)*65]`.
An empty `payload` (length 0) decodes to `(empty[], empty)`.

This shape is the project's reference choice. v2 / non-ECDSA encodings will re-shape `signatureData`
accordingly (e.g., a single aggregated BLS signature replaces the per-message concat; a
length-prefixed list accommodates variable-width schemes). The `IERC_BAM_Decoder` interface itself
imposes no constraint on `signatureData` layout — the layout is a contract between the decoder
implementation and the client's `splitSignatureData` helper for the matching signature scheme.
...
batch-registration event), `nonce` is the per-sender monotonically increasing counter (`uint64`, 8
bytes), and `contents` is the message content (`bytes`, variable length). The 60 fixed-size bytes
preceding `contents` make the `abi.encodePacked` encoding unambiguous despite the variable-length
tail.
...
ERC. The earlier `SocialBlobsCore` deployment remains for pre-amendment log
inspection; new registrations target the BAM core address above.

### Implementation divergence — scheme `0x01` (ECDSA)

The reference ECDSA-secp256k1 registry (`ECDSARegistry.sol`) takes a position
on two points the base `IERC_BAM_SignatureRegistry` interface leaves
under-specified for the "MAY allow keyless verification via ecrecover-style
key derivation" case. Both departures are intentional, live only on scheme
`0x01`, and do not change the interface signature:

1. **§Signature Registry Interface #8 — `verifyWithRegisteredKey` does not
   revert `NotRegistered`.** For an owner that has not called `register`,
   `verifyWithRegisteredKey(owner, hash, signature)` returns `true` iff
   `ecrecover(hash, signature) == owner` and the recovered address is
   non-zero, rather than reverting. The BLS-path semantics of "MUST revert
   `NotRegistered`" are preserved unchanged on scheme `0x02`.

2. **Test-case table — `isRegistered` returns `true` for every owner.**
   The base spec's test-case table implies `isRegistered(owner)` is `false`
   for an owner that has not taken a registration action. On the ECDSA path
   `isRegistered` is reinterpreted as "this registry can verify signatures
   for this address" and returns `true` for every address, because
   `ecrecover` can always verify for any 20-byte address.

**Rationale.** ECDSA verification on the EVM is address-native: `ecrecover`
is the verification primitive and yields a 20-byte address directly, so no
registration action is required to verify a signature against an owner's
own key. Treating "unregistered" as a revert would force every caller to
branch on registration state before verifying — exactly the branching this
registry exists to eliminate. `getKey(owner)` still returns empty bytes for
keyless owners (matching the ERC's test-case expectation); the "keyed vs
keyless" signal lives on the scheme-specific `hasDelegate(owner)` query
declared in `IECDSARegistry`, not on `isRegistered` or on `getKey` length.

Feeding this semantics back into the ERC itself — either as an explicit
carve-out for `ecrecover`-based schemes or as an amendment to the relevant
MUST clauses — is tracked in `BACKBURNER.md` #3.
...
message IDs; consumers that previously had to disambiguate cross-app duplicates externally now get
that separation from the ID itself.
