...
| [`bam-explorer`](apps/bam-explorer) | Read-only monitoring dashboard — surfaces every Reader and Poster read endpoint (health, status, pending, submitted batches, confirmed batches/messages per `contentTag`) on one fully client-rendered page, with per-batch drill-down |
