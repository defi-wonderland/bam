...
/**
 * Decode a batch from usable blob bytes, handling both old (v1) and new (v2) formats.
 */
export function decodeBlobBatch(usableBytes: Uint8Array) {
  ...
  patched.set(usableBytes.slice(0, 6), 0);
  patched[6] = 0x00;
  patched.set(usableBytes.slice(6), 7);
...
/**
 * Fetch raw blob data for a transaction via Beacon API or Blobscan.
 */
export async function fetchBlobForTx(txHash: string): Promise<Uint8Array | null> {
  ...
  if (blobscanResult) return blobscanResult;
...
async function fetchFromBlobscan(targetVersionedHash: string): Promise<Uint8Array | null> {
  try {
    ...
    const ref = (data.dataStorageReferences as Array<{ url?: string }>)?.[0];
    if (ref?.url) {
      const blobRes = await fetch(ref.url);
