...
contract BLSExposerTest is Test {
    ...
    function test_exposeFromCalldata_revertsOnSenderNotRegistered() public {
        address bob = address(0xB0B);
        bytes memory messageBytes = _buildMessage(bob, 1_700_000_000, 1, "test");

        // Bob has no BLS key registered
        vm.expectRevert(abi.encodeWithSelector(BLSExposer.SenderNotRegistered.selector, bob));
        exposer.exposeFromCalldata(_buildCalldataParams(messageBytes));
    }
    ...
    // ═══════════════════════════════════════════════════════════════════════════════
    // DIFFERENT MESSAGES SAME SENDER TESTS
    // ═══════════════════════════════════════════════════════════════════════════════
