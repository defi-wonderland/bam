...
describe('Calldata SDK Integration Tests', () => {
  ...
  describe('Batch Data Encoding', () => {
    ...
    it('should encode multiple message batch for calldata', () => {
      const messages = createSignedMessages([
        { sender: testAuthor, timestamp: testTimestamp, nonce: 0, content: 'First message' },
        { sender: testAuthor, timestamp: testTimestamp + 1, nonce: 1, content: 'Second message' },
        { sender: testAuthor, timestamp: testTimestamp + 2, nonce: 2, content: 'Third message' },
      ]);
    ...
    it('should compute content hash for calldata batch', () => {
      const messages = createSignedMessages([
        { sender: testAuthor, timestamp: testTimestamp, nonce: 0, content: 'Hash test message' },
      ]);
  ...
  describe('Message Offset Calculation', () => {
    ...
    it('should validate batch before encoding', () => {
      const messages = [
        { sender: testAuthor, timestamp: testTimestamp, nonce: 0, content: 'First' },
        { sender: testAuthor, timestamp: testTimestamp + 1, nonce: 1, content: 'Second message here' },
      ];
    ...
    it('should estimate batch size', () => {
      const messages = [
        { sender: testAuthor, timestamp: testTimestamp, nonce: 0, content: 'Target message content' },
      ];
    ...
    it('should encode and decode batch correctly', () => {
      ...
      expect(decoded.messages[0].sender.toLowerCase()).toBe(testAuthor.toLowerCase());
    });
  });
  ...
  describe('CalldataExposureParams Validation', () => {
    ...
    it('should create valid exposure params structure', () => {
      const messages = createSignedMessages([
        { sender: testAuthor, timestamp: testTimestamp, nonce: 0, content: 'Exposure test' },
      ]);
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle empty content message', () => {
      const messages = createSignedMessages([
        { sender: testAuthor, timestamp: testTimestamp, nonce: 0, content: '' },
      ]);
    ...
    it('should handle nonce values within 16-bit range', () => {
      const messages = createSignedMessages([
        { sender: testAuthor, timestamp: testTimestamp, nonce: 0xffff, content: 'Max nonce test' },
      ]);
    ...
    it('should handle unicode content in calldata', () => {
      const messages = createSignedMessages([
        { sender: testAuthor, timestamp: testTimestamp, nonce: 0, content: 'Hello 世界 🌍 مرحبا' },
      ]);
    ...
    it('should handle moderately long content', () => {
      ...
      const messages = createSignedMessages([
        { sender: testAuthor, timestamp: testTimestamp, nonce: 0, content: longContent },
      ]);
