...
describe('signECDSA (wallet path) vs signECDSAWithKey (headless)', () => {
  ...
  it('byte-identical signature for same (key, message, contentTag, chainId)', async () => {
    const msg: BAMMessage = { sender: ADDR, nonce: 7n, contents };
    const walletSig = await signECDSA(walletClient, msg, TAG);
    const headlessSig = signECDSAWithKey(PRIV, msg, TAG, foundry.id);
  ...
  it('both sigs verify via the same verifyECDSA call', async () => {
    ...
    const walletSig = await signECDSA(walletClient, msg, TAG);
    const headlessSig = signECDSAWithKey(PRIV, msg, TAG, foundry.id);
    expect(verifyECDSA(msg, TAG, walletSig, ADDR, foundry.id)).toBe(true);
    expect(verifyECDSA(msg, TAG, headlessSig, ADDR, foundry.id)).toBe(true);
  });

  it('wallet sig has v ∈ {27, 28} (normalisation applied)', async () => {
    for (const nonce of [0n, 1n, 10n, 100n]) {
      const sig = await signECDSA(walletClient, { sender: ADDR, nonce, contents }, TAG);
